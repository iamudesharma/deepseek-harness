...
import { constants, copyFileSync, cpSync, existsSync, fsyncSync, ftruncateSync, lstatSync, mkdirSync, openSync, closeSync, readdirSync, readFileSync, renameSync, rmSync, unlinkSync, writeFileSync, writeSync, } from 'node:fs';
import { basename, delimiter, dirname, isAbsolute, join, relative, resolve, sep } from 'node:path';
import { DESKTOP_PACKAGES_DIR, DESKTOP_PACKAGE_SET_FILE, DESKTOP_HOST_PACKAGE, desktopCorePackageOverrides, desktopDshPackageSpec, readDesktopCorePackageSet, verifyDesktopCorePackageSet, } from "./core-package-set.js";
import { parseDesktopRelease } from "./release.js";
import { extractPnpmStoreArchives, mergePnpmStore } from "./seed-store.js";
...
function copyMetadata(source, target) {
    ...
    for (const filename of DESKTOP_PROJECT_FILES) {
        ...
        if (existsSync(from))
            copyFileSync(from, join(target, filename), constants.COPYFILE_EXCL);
    }
...
function seedFiles(root) {
    ...
    const visit = (directory) => {
        for (const entry of readdirSync(directory, { withFileTypes: true })) {
            ...
            if (relativePath === 'integrity.json')
                continue;
            if (entry.isSymbolicLink())
                throw new Error(`desktop seed: symbolic link is not allowed: ${relativePath}`);
            if (entry.isDirectory()) {
                visit(path);
                continue;
            }
            if (!entry.isFile())
                throw new Error(`desktop seed: unsupported file type: ${relativePath}`);
...
export function verifySeedIntegrity(seedDir) {
...
export class DesktopProjectManager {
        ...
        switch (mutation.type) {
            case 'plugin-add': {
                ...
                if (requestedName === undefined)
                    throw new Error('desktop project: plugin package name is required');
    ...
    async activate(stagingProfile, hooks) {
        ...
        try {
            ...
            writeJson(this.paths.pending, { ...pending, step: 'active-moved' });
            if (existsSync(this.paths.profile)) {
                renameSync(this.paths.profile, this.paths.rollback);
                activeMoved = true;
            }
            mkdirSync(dirname(this.paths.profile), { recursive: true, mode: 0o700 });
            writeJson(this.paths.pending, { ...pending, step: 'staging-activated' });
...
export function createSeedMetadata(seedDir, release) {
    ...
    writeFileSync(join(seedDir, 'pnpm-workspace.yaml'), workspaceFile(desktopCorePackageOverrides(packageSet)), { mode: 0o600 });
