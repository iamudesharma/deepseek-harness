...
function developmentProject() {
    ...
    if (configured === undefined || configured === '')
        return undefined;
    if (app.isPackaged)
        throw new Error('dsh desktop: development project override is unavailable in packaged applications');
...
async function main() {
    ...
    protocol.handle(SCHEME, (request) => {
        ...
        if (url.hostname === 'shell')
            return serveShellAsset(request);
        if (url.hostname !== 'app')
    ...
    const createMainWindow = () => {
        ...
        mainWindow = window;
        window.once('ready-to-show', () => { if (!window.isDestroyed())
            window.show(); });
