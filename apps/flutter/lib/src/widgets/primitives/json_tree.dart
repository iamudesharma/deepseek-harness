...
///
/// Copy parity with `JsonTree.tsx` is tree-level: the header button copies
/// the whole value as pretty JSON (per-row value/path menus deferred).
/// Keyboard parity is expand/collapse: focused composite nodes answer
/// ArrowRight/ArrowLeft (roving Up/Down movement deferred to Tab order).
class DsJsonTree extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final String? prettyJson = _prettyJson;
        ...
        children: [
          if (prettyJson != null)
            Align(
              ...
              child: BlockCopyButton(copyText: prettyJson),
            ),
  ...
  /// Whole-tree pretty JSON for the copy button; null when unencodable.
...
class _JsonNodeState extends State<_JsonNode> {
  ...
  Widget build(BuildContext context) {
    ...
    // Composite with DisclosureRow. Focusable so ArrowRight/ArrowLeft
    // expand and collapse without a pointer (JsonTree.tsx expander keys;
    // Up/Down roving movement stays on Tab order).
    ...
    return Focus(
      onKeyEvent: (node, event) {
        ...
        if (event.logicalKey == LogicalKeyboardKey.arrowRight && !_open) {
          setState(() => _open = true);
          return KeyEventResult.handled;
        }
        if (event.logicalKey == LogicalKeyboardKey.arrowLeft && _open) {
