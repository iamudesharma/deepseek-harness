...
List<ToolCall> toolCallsFromHistory(List<HistoryEntry> entries) {
  ...
  return <ToolCall>[for (final String id in order) if (byId[id] != null) byId[id]!];
}

/// Provider for tool calls of a session.
///
/// Async with loading / error so [ToolCallTree] can render accordingly.
/// Sources from [connectionClientProvider] history; empty list while loading.
final toolCallsProvider =
    FutureProvider.family<List<ToolCall>, String>((ref, sessionId) async {
  final SessionId id = SessionId(sessionId);
  final ConnectionClient client = ref.watch(connectionClientProvider);
  try {
    final List<HistoryEntry> entries = await client.getSessionEvents(id);
    if (entries.isEmpty) return const <ToolCall>[];
    return toolCallsFromHistory(entries);
  } catch (e, st) {
...
final liveToolCallsProvider = Provider.family<List<ToolCall>, String>((ref, sessionId) {
