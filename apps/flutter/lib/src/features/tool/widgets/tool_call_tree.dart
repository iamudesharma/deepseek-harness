...
class ToolCallTree extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final AsyncValue<List<ToolCall>> async = ref.watch(toolCallsProvider(sessionId));
    final List<ToolCall> calls = live.isNotEmpty ? live : (async.valueOrNull ?? const <ToolCall>[]);
    if (calls.isNotEmpty) {
      return Container(
    ...
    return async.when(
      ...
      error: (Object err, StackTrace st) => _ToolErrorState(error: err.toString()),
    );
  }
}
...
class _ToolCallRowState extends ConsumerState<_ToolCallRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool expandable = widget.call.args.isNotEmpty || widget.call.result != null;
    final String title = '${widget.call.toolName} · ${_statusLabel(widget.call.status)}';
    ...
    return DisclosureRow(
      ...
      collapsedContent: Text(
        ...
        style: TextStyle(fontSize: DswTokens.fontSizeXxs12, color: aliases.labelCaption),
        overflow: TextOverflow.ellipsis,
      ),
      child: Padding(
        padding: const EdgeInsets.fromLTRB(DswTokens.spaceLg, DswTokens.spaceSm, DswTokens.spaceLg, DswTokens.spaceMd),
  ...
  String _collapsedPreview(ToolCall call) {
    ...
    if (args is Map && args.isNotEmpty) {
      final String? path = args['path'] as String? ?? args['file'] as String? ?? args['command'] as String?;
...
class GenericToolCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return _CardShell(children: <Widget>[
      _JsonBlock(label: 'Args', value: call.args),
      if (call.result != null) _JsonBlock(label: 'Result', value: call.result),
...
class ReadToolCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final String? path = call.args['path'] as String? ?? call.args['file'] as String?;
  ...
  Widget build(BuildContext context) {
    ...
    final DswAliases aliases =
        theme.extension<DswThemeExtension>()?.aliases ??
            (theme.brightness == Brightness.dark ? DswTokens.darkAliases : DswTokens.lightAliases);
    final String? path = call.args['path'] as String? ?? call.args['file'] as String?;
    final String patch = _stringify(call.result ?? call.args['patch'] ?? call.args['content'] ?? '');
    return _CardShell(children: <Widget>[
        ...
        Container(
          ...
          child: SelectableText(
            ...
            style: TextStyle(
              ...
              height: DswTokens.markdownCodeBlockSmallLineHeight / DswTokens.markdownCodeBlockSmallSize,
...
class SearchToolCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final String? query = call.args['pattern'] as String? ??
    ...
    return _CardShell(children: <Widget>[
      ...
      if (call.args['path'] case final String p?) _KVRow(keyLabel: 'Path', value: p),
      if (result.isNotEmpty) _CodeBlock(code: result, maxLines: 30) else _JsonBlock(label: 'Args', value: call.args),
    ]);
  }
}
...
class BashToolCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final DswAliases aliases =
        theme.extension<DswThemeExtension>()?.aliases ??
            (theme.brightness == Brightness.dark ? DswTokens.darkAliases : DswTokens.lightAliases);
    final String? command = call.args['command'] as String? ?? call.args['cmd'] as String?;
    final String result = _stringify(call.result);
    return _CardShell(children: <Widget>[
      if (command != null)
        Container(
          ...
          padding: const EdgeInsets.symmetric(horizontal: DswTokens.spaceSm, vertical: 6),
          child: SelectableText(
            '\$ $command',
            style: TextStyle(
      ...
      if (result.isNotEmpty) _CodeBlock(code: result, maxLines: 40) else if (command == null) _JsonBlock(label: 'Args', value: call.args),
    ]);
  }
}
    ...
    return Column(
      ...
      children: <Widget>[
        for (int i = 0; i < children.length; i++) ...<Widget>[
          children[i],
          if (i != children.length - 1) const SizedBox(height: DswTokens.spaceSm),
        ],
      ],
    );
  }
...
class _KVRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: <Widget>[
        Text('$keyLabel: ', style: TextStyle(fontSize: 12, color: aliases.labelTertiary, fontWeight: FontWeight.w600)),
        Expanded(child: SelectableText(value, style: TextStyle(fontSize: 12, color: aliases.labelSecondary))),
      ],
    );
  }
}
...
class _JsonBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        Text(label, style: TextStyle(fontSize: 12, color: aliases.labelTertiary, fontWeight: FontWeight.w600)),
...
class _CodeBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String display = truncated ? '${lines.take(maxLines).join('\n')}\n… (${lines.length - maxLines} more lines)' : code;
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(
        color: aliases.markdownCodeBlock,
    ...
    return Padding(
      ...
      child: Row(
        children: <Widget>[
          ...
          Expanded(child: Text(error, style: TextStyle(fontSize: 12, color: aliases.stateErrorPrimary))),
        ],
      ),
    );
  }
