...
class WorkspaceViewSnapshot {
  ...
  static WorkspaceViewSnapshot fromJson(Map<String, dynamic> json) {
    ...
    final groupBy = groupByStr == 'flat'
        ? SessionGroupBy.flat
        : SessionGroupBy.workspace;
    final orderBy = orderByStr == 'manual'
        ? SessionOrderBy.manual
        : SessionOrderBy.updated;
    final groupExpansion =
        (json['groupExpansion'] as Map?)?.map(
          (k, v) => MapEntry(k as String, v as bool),
        ) ??
        <String, bool>{};
    final sessionOrderByAccount =
        (json['sessionOrderByAccount'] as Map?)?.map(
          (k, v) =>
              MapEntry(k as String, (v as List).whereType<String>().toList()),
        ) ??
        <String, List<String>>{};
    final sessionUpdatedAtByAccount =
        (json['sessionUpdatedAtByAccount'] as Map?)?.map(
          (k, v) => MapEntry(
            k as String,
            (v as Map).map(
              (kk, vv) => MapEntry(kk as String, (vv as num).toInt()),
            ),
          ),
        ) ??
    ...
    return WorkspaceViewSnapshot(
      ...
      sessionOrderByAccount: Map<String, List<String>>.from(
        sessionOrderByAccount,
      ),
      sessionUpdatedAtByAccount: Map<String, Map<String, int>>.from(
...
Future<WorkspaceViewSnapshot?> loadWorkspaceView() async {
  try {
    ...
    if (decoded is Map<String, dynamic>)
      return WorkspaceViewSnapshot.fromJson(decoded);
    if (decoded is Map)
      return WorkspaceViewSnapshot.fromJson(decoded.cast<String, dynamic>());
  } catch (_) {}
  return null;
}
...
final workspaceViewPersistenceProvider = Provider<void>((ref) {
  ...
  loadWorkspaceView().then((snapshot) {
    ...
    ref.read(workspaceExpandedProvider.notifier).state =
        snapshot.groupExpansion;
    ref.read(workspaceSessionOrderProvider.notifier).state =
        snapshot.sessionOrderByAccount;
    ...
    ref.read(workspaceSessionUpdatedAtProvider.notifier).state =
        snapshot.sessionUpdatedAtByAccount;
  });
  ...
  void save() {
    final snap = WorkspaceViewSnapshot(
      ...
      groupExpansion: Map<String, bool>.from(
      ...
      sessionUpdatedAtByAccount: Map<String, Map<String, int>>.from(
        ref.read(workspaceSessionUpdatedAtProvider),
      ),
    );
  ...
  ref.listen<Map<String, List<String>>>(
    workspaceSessionOrderProvider,
    (_, __) => save(),
  );
  ref.listen<Map<String, Map<String, int>>>(
    workspaceSessionUpdatedAtProvider,
    (_, __) => save(),
  );
});
...
final workspaceSessionUpdatedAtProvider =
