...
final seatDirectoryProvider = StateNotifierProvider.family<ModelDirectory,
    ModelDirectoryState, String>((ref, sessionId) {
...
class ModelSeat extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final directory =
        ref.read(seatDirectoryProvider(session.sessionId.value).notifier);
    // bindLocale watches localeRevisionProvider so the seat copy follows a
    // Language-row switch.
    return _seat(context, ref, ref.bindLocale(kModelNamespace), directory,
        state, session.sessionId.value);
  }
  ...
  Widget _seat(
    ...
    for (final g in state.groups) {
      for (final m in g.models) {
        if (current != null && g.id == current.provider && m.id == current.model) {
          currentModel = m;
          break;
        }
      }
    ...
    final effectiveEffort = current?.reasoningEffort ?? reasoning?.defaultEffort;
    String? effortLabel;
    if (reasoning != null) {
      if (effectiveEffort == null) {
        effortLabel = t('effort.providerDefault');
      } else {
        final match = reasoning.efforts.where((e) => e.id == effectiveEffort).firstOrNull;
        effortLabel = match?.name ?? effectiveEffort;
      }
    }

    final String label =
        current == null ? t('trigger.fallback') : currentModel?.name ?? current.model;
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      PopupMenuButton<(String, ModelInfo)>(
        ...
        onOpened: () {
          if (state.status == 'idle' || state.status == 'error') {
            directory.load().catchError((Object _) => const <String, dynamic>{});
          }
        },
              ...
              PopupMenuItem<(String, ModelInfo)>(
                ...
                child: Row(children: [
                  ...
                  if (current?.provider == group.id && current?.model == model.id)
                    Icon(Icons.check,
                        size: 14, color: aliases.stateBusinessPrimary),
                ]),
              ),
        ...
        onSelected: ((String, ModelInfo) pick) async {
          if (current != null && current.provider == pick.$1 && current.model == pick.$2.id) return;
          final target = pick.$2;
          ...
          if (!accepted && context.mounted) {
            final err = ref.read(seatDirectoryProvider(sessionId).notifier).state.error ?? t('error.action');
            ref.read(toastProvider.notifier).showError(err);
          }
        },
        child: Container(
          padding: const EdgeInsets.symmetric(
              horizontal: DswTokens.spaceSm, vertical: 4),
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.memory_outlined, size: 12, color: aliases.labelSecondary),
      ...
      if (reasoning != null) ...[
        ...
        PopupMenuButton<String?>(
          ...
          itemBuilder: (BuildContext context) {
            ...
            if (reasoning.defaultEffort == null) {
              items.add(PopupMenuItem<String?>(
                value: null,
                child: Row(children: [
                  Expanded(child: Text(t('effort.providerDefault'), style: TextStyle(fontSize: DswTokens.fontSizeS14, color: aliases.labelPrimary))),
                  if (effectiveEffort == null) Icon(Icons.check, size: 14, color: aliases.stateBusinessPrimary),
                ]),
              ));
            }
            for (final e in reasoning.efforts) {
              items.add(PopupMenuItem<String?>(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Expanded(child: Text(e.name, style: TextStyle(fontSize: DswTokens.fontSizeS14, color: aliases.labelPrimary))),
                    if (effectiveEffort == e.id) Icon(Icons.check, size: 14, color: aliases.stateBusinessPrimary),
                  ]),
                  if (e.description != null)
                    Text(e.description!, style: TextStyle(fontSize: DswTokens.fontSizeXxs12, color: aliases.labelTertiary)),
                ]),
              ));
            }
            if (items.isEmpty) {
          ...
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: DswTokens.spaceSm, vertical: 4),
            decoration: BoxDecoration(
              color: aliases.bgOverlay,
              borderRadius: BorderRadius.circular(DswTokens.radiusFull),
              border: Border.all(color: aliases.borderL2),
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.psychology_outlined, size: 12, color: aliases.labelSecondary),
              const SizedBox(width: 4),
              Text(effortLabel ?? t('menu.effort'),
                  style: TextStyle(fontSize: DswTokens.fontSizeXxs12, fontWeight: FontWeight.w600, color: aliases.labelSecondary)),
              const Icon(Icons.expand_more, size: 12),
            ]),
          ),
        ),
