...
void main() {
  group('DsConnectionBanner (ConnectionBanner.tsx parity)', () {
    testWidgets('connected and idle render nothing', (tester) async {
      await tester.pumpWidget(_wrap(const DsConnectionBanner(state: conn.ConnectionState.connected)));
      expect(find.text('Connecting'), findsNothing);
      expect(find.text('Reconnecting'), findsNothing);
      expect(find.text('Disconnected'), findsNothing);

      await tester.pumpWidget(_wrap(const DsConnectionBanner(state: conn.ConnectionState.idle)));
      expect(find.text('Connecting'), findsNothing);
    });

    testWidgets('reconnecting shows busy spinner and retry copy', (tester) async {
      await tester.pumpWidget(_wrap(const DsConnectionBanner(state: conn.ConnectionState.reconnecting)));
      expect(find.text('Reconnecting'), findsOneWidget);
      expect(find.text('Connection lost — retrying…'), findsOneWidget);
      expect(find.byType(CircularProgressIndicator), findsOneWidget);
    });

    testWidgets('disconnected shows offline copy without spinner', (tester) async {
      await tester.pumpWidget(_wrap(const DsConnectionBanner(state: conn.ConnectionState.disconnected)));
  ...
  group('DsJsonTree (JsonTree.tsx parity: collapsible typed tree)', () {
    testWidgets('expanded root renders leaf key and type-tinted value', (tester) async {
      await tester.pumpWidget(_wrap(const DsJsonTree(
    ...
    testWidgets('collapsed composite shows child-count summary, tap expands children', (tester) async {
      await tester.pumpWidget(_wrap(const DsJsonTree(data: <String, dynamic>{
        'a': 1,
        'b': <String>['x'],
      })));
    ...
    testWidgets('initiallyExpanded reveals nested arrays without interaction', (tester) async {
      await tester.pumpWidget(_wrap(const DsJsonTree(
        data: <String, dynamic>{
          'list': <String>['x'],
        },
  ...
  group('DsHoverCard (HoverCard.tsx parity: delayed reachable preview card)', () {
    ...
    testWidgets('leaving the anchor closes after the grace window', (tester) async {
    ...
    testWidgets('copyText adds a copy row that reports success', (tester) async {
      var copied = false;
      await pumpAnchor(tester, copyText: '/long/path', onCopy: () => copied = true);
      await tester.pump(const Duration(milliseconds: 450));
      await tester.pumpAndSettle();
      ...
      final InkWell copyRow = tester.widget<InkWell>(
        find.ancestor(of: find.text('Copy'), matching: find.byType(InkWell)).first,
      );
      copyRow.onTap?.call();
      await tester.pump();
  ...
  group('DsOnboardingSurface (OnboardingSurface.tsx parity: takeover chrome)', () {
    const steps = <DsOnboardingStep>[
      DsOnboardingStep(title: 'Welcome', description: 'First step', ctaLabel: 'Next'),
      DsOnboardingStep(title: 'Done', description: 'Last step'),
    ];

    testWidgets('mid-flow shows step copy, skip affordance, and advances via CTA', (tester) async {
      var next = 0;
      var skipped = false;
      await tester.pumpWidget(_wrap(DsOnboardingSurface(
        steps: steps,
        currentStep: 0,
        onNext: () => next++,
        onSkip: () => skipped = true,
    ...
    testWidgets('last step swaps CTA for completion and drops skip', (tester) async {
      var completed = false;
      await tester.pumpWidget(_wrap(DsOnboardingSurface(
        steps: steps,
        currentStep: 1,
  ...
  group('DsIcons (icons/index.tsx substitution onto Material glyphs)', () {
    testWidgets('mapped factories render their Material glyph at the requested size', (tester) async {
      ...
      final Icon warning = tester.widget<Icon>(find.byIcon(Icons.warning_amber_rounded));
  ...
  group('DsMarkdown + sanitizeUrl (markdown/render.tsx allowlist parity)', () {
    test('sanitizeUrl allows http/https/mailto and blocks script and data schemes', () {
    ...
    testWidgets('renders gfm headings, emphasis, and link text', (tester) async {
      await tester.pumpWidget(_wrap(const DsMarkdown(
        data: '# Heading\n\n**bold** text',
      )));
    ...
    testWidgets('tapping an https link routes through openExternal without crashing', (tester) async {
      await tester.pumpWidget(_wrap(const DsMarkdown(data: '[good](https://example.com)')));
    ...
    testWidgets('a javascript: link never reaches the launcher', (tester) async {
      await tester.pumpWidget(_wrap(const DsMarkdown(data: '[bad](javascript:void(0))')));
