...
Widget _pumpAppFrame({required Size size, List<Override> overrides = const []}) {
...
void main() {
  group('AppFrame', () {
    testWidgets('expanded at 1200x800: sidebar visible, drag handle exists, not collapsed', (tester) async {
    ...
    testWidgets('collapsed at 400x800: sidebar collapsed to rail, conversation still visible', (tester) async {
    ...
    testWidgets('wide shows drag handle, narrow hides sidebar handle', (tester) async {
      // Start wide
      tester.view.physicalSize = const Size(1200, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets('with sidebar and conversation slots renders provided widgets', (tester) async {
      tester.view.physicalSize = const Size(1200, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      addTearDown(() => tester.view.resetDevicePixelRatio());
      ...
      expect(find.text('Custom Details', skipOffstage: false), findsOneWidget);
      // User-visible: details text should NOT be visible when collapsed
      expect(find.text('Custom Details'), findsNothing);
    });

    testWidgets('details track boots closed; no details handle without an open details panel', (tester) async {
    ...
    testWidgets('AppFrame respects ProviderScope layout override for collapsed state', (tester) async {
      tester.view.physicalSize = const Size(1200, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      addTearDown(() => tester.view.resetDevicePixelRatio());
