  ...
  group('AttachmentRail', () {
    ...
    testWidgets('renders one card per attachment', (tester) async {
      ...
      await tester.pumpWidget(
        wrap(AttachmentRail(items: data, onOpen: (_) {}, onRemove: (_) {})),
      );
      // Each rail item hosts an InkWell for the thumbnail (open).
      expect(find.byType(InkWell), findsAtLeastNWidgets(2));
    });

    testWidgets('single-click open invokes onOpen with the attachment', (
      tester,
    ) async {
      final data = items(1);
    ...
    testWidgets('remove button invokes onRemove with the attachment', (
      tester,
    ) async {
      final data = items(1);
    ...
    testWidgets('when onRemove is null, no close button is rendered', (
      tester,
    ) async {
      final data = items(1);
      await tester.pumpWidget(
        wrap(AttachmentRail(items: data, onOpen: (_) {}, onRemove: null)),
      );
      expect(find.byIcon(Icons.close), findsNothing);
    });

    testWidgets('lightbox shows placeholder when no preview url', (
      tester,
    ) async {
    ...
    testWidgets(
      'native file thumbnail renders without dart:io crash (conditional seam)',
      (tester) async {
    ...
    testWidgets('http previewUrl renders Image.network', (tester) async {
      final data = [
        const ComposerAttachment(
          name: 'remote.png',
          previewUrl: 'http://example.com/a.png',
          mimeType: 'image/png',
        ),
      ];
      await tester.pumpWidget(
    ...
    testWidgets('lightbox with http preview shows network image', (
      tester,
    ) async {
    ...
    testWidgets(
      'lightbox with file path uses native seam fallback placeholder',
      (tester) async {
