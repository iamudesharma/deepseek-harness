...
void main() {
  group('Tool/visual parity completion', () {
    testWidgets(
      'ToolNode bare row + ioCard r12 borderL1 markdownCodeBlock with AnimatedSize',
      (tester) async {
        ...
        final Container ioCard = tester.widget<Container>(
          find
              .descendant(
        ...
        expect(
          dec.border?.top.color,
          buildLightTheme().extension<DswThemeExtension>()!.aliases.borderL1,
        );
        expect(
          dec.color,
          buildLightTheme()
        ...
        for (final c in containers) {
          ...
          if (d?.borderRadius == BorderRadius.circular(DswTokens.radiusSm) &&
              d?.border?.top.color ==
                  buildLightTheme()
                      .extension<DswThemeExtension>()!
                      .aliases
    ...
    testWidgets('block-family unified radius 12 headTailCap 16 copy/expand', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        _wrap(
          const SingleChildScrollView(
            child: DsCodeBlock(code: 'const x=1;', language: 'ts'),
          ),
        ),
      );
      await tester.pump();
      ...
      final codeFrame = tester.widget<Container>(
        find
            .descendant(
    ...
    testWidgets('composer 748 measure floating r22 capsule + blue send disc', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        _wrap(const ConversationComposer(sessionId: 's-1')),
      );
      await tester.pump();
      ...
      for (final c in containers) {
        final d = c.decoration as BoxDecoration?;
        if (d?.borderRadius == BorderRadius.circular(22)) {
          card = c;
          break;
        }
      }
    ...
    testWidgets(
      'AppFrame slide 300 ease slide but freeze lastWideWidth + 150 crossfade + reduced-motion gate',
      (tester) async {
        ...
        // Full motion
        await tester.pumpWidget(
          _wrap(const Scaffold(body: AppFrame()), reduceMotion: false),
        );
        await tester.pump();
        ...
        final anySlow = animatedContainers.any(
          (c) => c.duration == DswTokens.transitionDurationSlow,
        );
        expect(anySlow, isTrue);
        // Reduced motion => duration zero
        await tester.pumpWidget(
          _wrap(const Scaffold(body: AppFrame()), reduceMotion: true),
        );
        await tester.pump();
        await tester.pump(const Duration(milliseconds: 200));
        // Debug MediaQuery
        ...
        // ignore: avoid_print
        print(
          'prefersReducedMotion: ${prefersReducedMotion(ctx)} disableAnimations: ${MediaQuery.maybeOf(ctx)?.disableAnimations}',
        );
        final reducedContainers = tester.widgetList<AnimatedContainer>(
          find.byType(AnimatedContainer),
        );
        // Debug: print durations
        // ignore: avoid_print
        print(
          'reducedContainers durations: ${reducedContainers.map((c) => c.duration).toList()}',
        );
    ...
    testWidgets('AppFrame scrollbar linger 2000ms + 36 rail controls', (
      tester,
    ) async {
