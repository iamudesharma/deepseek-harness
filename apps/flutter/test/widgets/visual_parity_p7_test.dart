...
Widget _wrap(Widget child, {bool reduceMotion = false, ThemeData? theme}) {
  return ProviderScope(
    child: MaterialApp(
      ...
      home: MediaQuery(
        data: MediaQueryData(disableAnimations: reduceMotion, size: const Size(800, 600)),
...
void main() {
  group('P7 visual/system parity', () {
    testWidgets('DsModalOverlay shows BackdropFilter blur(2) + bgMask1', (tester) async {
    ...
    testWidgets('DsModalOverlay Esc closes (CallbackShortcuts)', (tester) async {
      var closed = false;
      await tester.pumpWidget(_wrap(DsModalOverlay(
        open: true,
        title: 'Esc test',
        onClose: () => closed = true,
        child: const Text('x'),
    ...
    testWidgets('StateDot ongoing halts under reduced-motion', (tester) async {
      await tester.pumpWidget(_wrap(const StateDot(state: StateDotState.ongoing), reduceMotion: true));
      await tester.pump(const Duration(milliseconds: 200));
      expect(
        find.descendant(of: find.byType(StateDot), matching: find.byType(CustomPaint)),
        findsNothing,
      );
    });

    testWidgets('DsMenu regular row minHeight is 40 (dense 34 compact 26)', (tester) async {
      ...
      await tester.pumpWidget(_wrap(DsMenu(
    ...
    testWidgets('DsRiskConfirmation uses 440 width, error icon, buttonPrimaryFill', (tester) async {
      await tester.pumpWidget(_wrap(DsRiskConfirmation(
        open: true,
        title: 'Enable?',
        description: 'Risky',
      ...
      final overlay = tester.widget<DsModalOverlay>(find.byType(DsModalOverlay));
      expect(overlay.width, 440);
      // Warning row icon should be error color, not warn
      final icons = tester.widgetList<Icon>(find.byType(Icon));
      final hasWarning = icons.any((i) => i.icon == Icons.warning_amber_rounded && i.size == 18);
    ...
    testWidgets('DisclosureRow reduced-motion disables AnimatedSize duration', (tester) async {
    ...
    testWidgets('DsToastHost skips slide under reduced-motion', (tester) async {
      await tester.pumpWidget(_wrap(const DsToastHost(child: Text('root')), reduceMotion: true));
      final container = ProviderScope.containerOf(tester.element(find.byType(DsToastHost)));
    ...
    testWidgets('DsTerminalBlock uses alias background radius 12 borderL1', (tester) async {
      await tester.pumpWidget(_wrap(const DsTerminalBlock(
        command: 'echo hi',
        output: 'hi',
      )));
      await tester.pump();
      final container = tester.widget<Container>(find.descendant(of: find.byType(DsTerminalBlock), matching: find.byType(Container)).first);
    ...
    testWidgets('prefersReducedMotion helper mirrors disableAnimations', (tester) async {
