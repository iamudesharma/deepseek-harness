...
void main() {
  testWidgets('prefersReducedMotion reflects disableAnimations', (
    ...
    Widget probe = Builder(
      builder: (context) =>
          Text(prefersReducedMotion(context) ? 'reduced' : 'full'),
    );
  ...
  testWidgets('StateDot ongoing still animates when motion is full', (
    ...
    await tester.pumpWidget(
      _harness(const StateDot(state: StateDotState.ongoing)),
    );
    await tester.pump(const Duration(milliseconds: 300));
    expect(find.byWidgetPredicate((w) => w is CustomPaint), findsWidgets);
  });
}
