...
HistoryEntry _h(
  ...
  event: _ev(type, data, seq: seq, time: time),
  view: view,
);
...
void main() {
  group('ConversationReducer', () {
    ...
    test(
      '2. streaming response: one assistant message updates incrementally',
      () {
        final r = reduceConversation([
        ...
        final assistants = r.messages
            .where((m) => m.role.toString().contains('assistant'))
            .toList();
    ...
    test('3. thinking/reasoning rendered as collapsible thinking section', () {
      ...
      final assistant = r.messages.firstWhere(
        (m) => m.role.toString().contains('assistant'),
      );
    ...
    test(
      '4. tool call + tool result: tool card pending/running -> completed',
      () {
        final r = reduceConversation([
      ...
      final r = reduceConversation([
        ...
        _h('turn/end', {
          'reason': {
            ...
            'error': {
              'type': 'ModelError',
              'message': '401 ModelError: Free promotion has ended for DeepSeek V4 Flash Free',
            },
          },
        }, seq: 4),
      ]);
      expect(
        r.tools.where((t) => t.status.toString() == 'ToolCallStatus.running'),
        isEmpty,
      );
      expect(r.tools, hasLength(3));
      // unresolved tools become cancelled / not-executed
      expect(
        r.tools.every((t) => t.status.toString().contains('cancel')),
        isTrue,
      );
    ...
    test('7. model failure after some tools complete: completed stay, unresolved cancelled', () {
      ...
      expect(
        r.tools
            .where((t) => t.id == 'c2' || t.id == 'c3')
            .every((t) => t.status.toString().contains('cancel')),
        isTrue,
      );
    });
