...
class QaClient extends conn.ConnectionClient {
  ...
  ({String method, Map<String, dynamic> payload})? singleCall(String method) {
    final hits = calls.where((c) => c.method == method).toList(growable: false);
...
void main() {
  testWidgets(
    'model dropdown selects strictly from groups and sends the advertised default effort',
    (tester) async {
  ...
  testWidgets(
    'effort submenu appears only for a reasoning model and preserves provider/model',
    (tester) async {
  ...
  testWidgets(
    'empty catalog renders the explicit empty row and never synthesizes a selection',
    (tester) async {
      final client = QaClient()
  ...
  testWidgets(
    'permission seat gates danger-full-access then submits /permission through the command channel',
    (tester) async {
      final client = QaClient();
  ...
  testWidgets(
    "typing '/' opens the trigger menu and picking a host command executes it detached",
    (tester) async {
      final client = QaClient();
  ...
  testWidgets(
    'submit carries draft text and staged image parts through the send channel',
    (tester) async {
      final client = QaClient();
      final container = await pumpComposedShell(tester, client);
  ...
  testWidgets(
    'tool row order: access < plan < model < send inside the composer card',
    (tester) async {
      final client = QaClient();
      final container = await pumpComposedShell(tester, client);
  ...
  testWidgets("typing '/' anchors the trigger menu to the composer card", (
    ...
    final Rect menu = tester.getRect(
      find.byKey(const ValueKey('input-menu-surface')),
    );
