...
ProviderContainer _containerWithEvents(
  String sid,
  ...
  c
      ...
      .replaceAll(
        events.map((e) => HistoryEntry(event: e, view: null)).toList(),
      );
  return c;
}
...
void main() {
  group('Chat goldens', () {
    testWidgets('chat with assistant text', (tester) async {
      await _pumpChat(tester, 'gold-assistant', [
    ...
    testWidgets('chat with Bash', (tester) async {
      await _pumpChat(tester, 'gold-bash', [
    ...
    testWidgets('chat with Read', (tester) async {
      await _pumpChat(tester, 'gold-read', [
    ...
    testWidgets('chat with Search', (tester) async {
      await _pumpChat(tester, 'gold-search', [
        ...
        _ev('tool/result', 3, {
          ...
          'result': 'lib/a.dart:10: foo bar\nlib/b.dart:20: baz foo',
        }),
      ], 'chat_search');
    });

    testWidgets('tool error', (tester) async {
      await _pumpChat(tester, 'gold-tool-error', [
    ...
    testWidgets('nested subcall', (tester) async {
      await _pumpChat(tester, 'gold-subcall', [
        _ev('turn/start', 1, {'turn': 1}),
    ...
    testWidgets('two steps', (tester) async {
      await _pumpChat(tester, 'gold-two-steps', [
        _ev('turn/start', 1, {'turn': 1}),
        _ev('step/start', 2, {'turn': 1, 'step': 1}),
        _ev('assistant/chunk', 3, {
          'turn': 1,
        ...
        _ev('assistant/message', 4, {
          ...
          'message': {
            'content': [
              {'type': 'text', 'text': 'First step thinking'},
            ],
          },
        }),
        ...
        _ev('assistant/chunk', 9, {
          'turn': 1,
          'step': 2,
          'chunk': {'type': 'text-delta', 'index': 0, 'text': 'Second step'},
        }),
      ...
      for (int s = 1; s <= 4; s++) {
        ...
        events.add(
          _ev('assistant/chunk', seq++, {
            'turn': 1,
            'step': s,
            'chunk': {'type': 'text-delta', 'index': 0, 'text': 'Step $s'},
          }),
        );
        events.add(
    ...
    testWidgets('final settled state', (tester) async {
      await _pumpChat(tester, 'gold-settled', [
