...
void main() {
  group('sessionCreatePayload', () {
    test('carries the workspace binding only when declared', () {
      expect(
        sessionCreatePayload(workspaceId: 'ws-1'),
        {'workspaceId': 'ws-1'},
      );
  ...
  group('isWorkspaceAttachFailure', () {
    test('matches the workspace-not-found wire literal in transport errors',
        () {
      expect(
        isWorkspaceAttachFailure(Exception('session.create: '
  ...
  group('adoptHostBornSession', () {
    test('projects a blank, idle summary — entity birth precedes first message',
        () {
