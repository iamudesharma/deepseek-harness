...
void main() {
  test(
    'builtin registry mirrors BUILTIN_THEMES (light/dark, empty overrides)',
    () {
  ...
  test(
    'system resolves through platform brightness and re-resolves on change',
    () {
      final runtime = ThemeRuntime(
  ...
  test('registerTheme appends and rejects duplicates', () {
    final runtime = ThemeRuntime()
      ..registerTheme(
        const ThemeDefinition(id: 'ocean', colorScheme: ColorSchemeBase.dark),
      );
    expect(runtime.snapshot.themes.map((t) => t.id), contains('ocean'));
    expect(
      () => runtime.registerTheme(
        const ThemeDefinition(id: 'ocean', colorScheme: ColorSchemeBase.light),
      ),
      throwsArgumentError,
    );
  });
  ...
  test('override tokens require both modes and fold per active scheme', () {
    ...
    expect(
      runtime.snapshot.active.tokens['--dsw-alias-brand-primary'],
      isNotEmpty,
    );
