...
void main() {
  ...
  group('WorkspacesService wire mapping', () {
    test('list/create/archiveSession ride the workspace.* methods', () async {
      ...
      expect(await svc.create(path: '/repo'), {
        'id': 'ws-2',
        'path': '/repo',
      });
    ...
    test('picker and browse ride the host.* methods; abort supersedes',
        () async {
      final client = _RecordingClient()
      ...
      expect(client.calls,
          ['host.pickDirectory', 'host.listDirectory']);
    ...
    test('createDirectory falls back through nested and missing paths',
        () async {
      final client = _RecordingClient()
      ...
      expect(
        await WorkspacesService(client).createDirectory(
            path: '/nested', name: 'answer'),
        '/nested/answer',
      );
    });
  });
