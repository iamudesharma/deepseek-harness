...
SessionEventEnvelope e(String type, int seq, Map<String, Object?> data,
        {List<int>? sources, Object? surfaceOp}) =>
...
void main() {
  test('manual compaction lifecycle: run -> summary -> checkpoint -> done yields manual node with compaction', () {
    final f = ConversationNodeFolder()
      ..add(e('command/run', 1, {'commandId': 'cmd-1', 'name': 'compact', 'args': ''}))
      ..add(SessionEventEnvelope.fromJson({
        ...
        'data': {
          ...
          'summary': [{'type': 'text', 'text': 'manual summary **bold**'}],
          'shadowedSeqs': [10, 11],
      ...
      ..add(e('command/done', 4, {'commandId': 'cmd-1', 'kind': 'success', 'text': 'done', 'sourceEventSeq': 2}));
  ...
  test('manual compaction out-of-order parent: child before parent still materializes correctly', () {
    final f = ConversationNodeFolder()
      ...
      ..add(e('tool/code-dispatch-start', 2, {
        'rootCallId': 'root-1',
      ...
      ..add(e('tool/code-dispatch-start', 3, {
        'rootCallId': 'root-1',
        'parentCallId': 'root-1',
  ...
  test('depth 256 cap rejects', () {
    ...
    for (int i = 1; i <= 255; i++) {
      ...
      final child = 'c${i+1}';
      f.add(e('tool/code-dispatch-start', i+1, {
        'rootCallId': 'root-1',
        'parentCallId': parent,
        'subCallId': child,
        'name': 'n',
    ...
    f.add(e('tool/code-dispatch-start', 300, {
      'rootCallId': 'root-1',
      'parentCallId': 'c256',
      'subCallId': 'c257',
      'name': 'n',
    ...
    final tool = f.snapshot().nodes.whereType<ToolNode>().single;
    int maxDepth(ToolSubCall n) => n.children.isEmpty ? 1 : 1 + n.children.map(maxDepth).reduce((a,b)=> a>b?a:b);
  ...
  test('cycle rejection', () {
    ...
    f.add(e('tool/code-dispatch-start', 4, {
      'rootCallId': 'root-1',
      'parentCallId': 'b',
      'subCallId': 'a',
      'name': 'x',
