...
void main() {
  ...
  test(
    'eleven WS-Surfaces plugins activate and publish their services',
    () async {
  ...
  test('hero contributions wait-and-follow their hole declarations', () async {
    ...
    expect(
      host.slots.winnersOfSlot('conversation.hero.brand.mark'),
      hasLength(1),
    );
    expect(
      host.slots.winnersOfSlot('conversation.hero.workspace'),
      hasLength(1),
    );
  });
  ...
  test('model seat occupies the declared input.model hole', () async {
    ...
    final directories = host.service<ModelDirectoryService>(
  ...
  test(
    'attachment staging adds once, removes, and clears with teardown',
    () async {
      final host = wsSurfacesHost();
      addTearDown(host.deactivateAll);
  ...
  test(
    'settings children expose scope-backed services over the wire face',
    () async {
      final client = FakeClient();
      final host = wsSurfacesHost(client: client);
      addTearDown(host.deactivateAll);
      ...
      final general = host.service<GeneralSettingsService>(
      ...
      final plugins = host.service<PluginsSettingsService>(
      ...
      final models = host.service<ModelsSettingsService>(
