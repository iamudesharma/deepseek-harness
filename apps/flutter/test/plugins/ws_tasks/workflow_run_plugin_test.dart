...
const _run = WorkflowRunChatData(
  ...
  phases: [
    WorkflowRunPhaseData(key: 'value:6:review', phase: 'review', members: [
      WorkflowRunMemberData(
          seq: 1, label: 'reviewer-a', childId: 'sess-child-1',
          status: WorkflowRunStatus.completed),
      WorkflowRunMemberData(
          seq: 2, label: 'reviewer-b', childId: 'sess-child-2',
          status: WorkflowRunStatus.running),
    ]),
    WorkflowRunPhaseData(key: 'missing', phase: null, members: [
      WorkflowRunMemberData(
          seq: 3, label: '', childId: 'sess-child-3',
...
void main() {
  test('activation registers the keyed renderer and the dictionaries', () async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: Builder(builder: (context) {
          final renderer = controller.renderers.resolve(kWorkflowRunNodeKey)!;
  ...
  testWidgets('the panel discloses run and phases with status-driven defaults',
      (tester) async {
    final opened = <String>[];
    final host = wsTasksHost();
    addTearDown(host.deactivateAll);
