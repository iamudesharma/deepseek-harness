...
void main() {
  test('activation installs the header action after the subagent catalog and leaves on deactivation',
      () async {
    ...
    final ids = [
      for (final w in host.slots.winnersOfSlot('conversation.session.header.actions'))
        w.options.id
    ];
  ...
  test('rows order live-first by start, settled newest-first; forged statuses fail loud', () {
    final rows = [
      JobViewRow.fromJson(rawJob(id: 'bash-2', label: 'settled old',
          status: 'completed', startedAt: 1_000, finishedAt: 5_000)),
      JobViewRow.fromJson(rawJob(id: 'bash-3', label: 'live late',
          status: 'running', startedAt: 9_000)),
      JobViewRow.fromJson(rawJob(id: 'bash-1', label: 'live early',
          status: 'stopping', startedAt: 2_000)),
      JobViewRow.fromJson(rawJob(id: 'bash-4', label: 'settled new',
          status: 'failed', startedAt: 3_000, finishedAt: 8_000, detail: 'exit code: 1')),
    ];
    expect([for (final r in ordered(rows)) r.id],
  ...
  testWidgets('a seeded jobsProvider shows the chip and opens the row list',
      ...
      overrides: [
        jobsProvider.overrideWith((ref) {
          ...
          controller.replace('s1', [
            rawJob(id: 'bash-1', label: 'pnpm run test:coverage',
                status: 'running', startedAt: now - 65_000),
            rawJob(id: 'bash-2', label: 'pnpm run lint',
                status: 'failed', startedAt: now - 200_000,
                finishedAt: now - 120_000, detail: 'exit code: 1'),
          ]);
          return controller;
        }),
        currentSessionIdProvider.overrideWithValue(const SessionId('s1')),
    ...
    expect(find.text('failed'), findsNothing); // detail replaces the status word
