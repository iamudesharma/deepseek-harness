...
void main() {
  ...
  test('mentions resolve by exact path, then unique basename; ambiguity stays inert', () async {
    final host = wsTasksHost();
    addTearDown(host.deactivateAll);
    host.register(const DeliverablesPlugin());
    await host.activateAll();
    ...
    expect(
      mentions
          .resolve(paths: paths, token: 'src/app.css', openFile: open)!
          .path,
      'src/app.css',
    );
    expect(
      mentions.resolve(paths: paths, token: 'index.html', openFile: open)!.path,
      'out/index.html',
    );
    // A basename two produced paths share never guesses.
    expect(
      mentions.resolve(paths: paths, token: 'README.md', openFile: open),
      isNull,
    );
    expect(
      mentions.resolve(paths: paths, token: 'nope.txt', openFile: open),
      isNull,
    );
