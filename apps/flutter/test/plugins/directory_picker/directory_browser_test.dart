...
void main() {
  group('DirectoryBrowser Miller columns', () {
    ...
    testWidgets(
      'opens at Host home as one wide column, hides hidden entries, roots crumbs at Home',
      (tester) async {
        await tester.pumpWidget(
          wrap(
            DirectoryBrowser(
    ...
    testWidgets(
      'selects a row into two-pane view: children preview right, crumbs follow selection',
      (tester) async {
        await tester.pumpWidget(
          wrap(
            DirectoryBrowser(
    ...
    testWidgets('advances one level when right-column row is picked', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets('crumb jump to Home lands single wide level (display root)', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets(
      'prefix-filters listed level from draft tail, dot reveals hidden',
      (tester) async {
        await tester.pumpWidget(
          wrap(
            DirectoryBrowser(
    ...
    testWidgets(
      'Open adopts selected folder, else listed level; Cancel closes',
      (tester) async {
        String? opened;
        bool closed = false;
        await tester.pumpWidget(
    ...
    testWidgets('New folder creates and selects the created folder', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(
      ...
      await tester.enterText(
        find.byKey(const ValueKey('createFolderInput')),
        'fresh',
      );
      await tester.pump();
      ...
      expect(
        find.text('fresh'),
        findsNothing,
      ); // not in fake tree, but flow completed without error
    });
    ...
    testWidgets('shows truncated indicator when level cut', (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('Too many folders to list; only the beginning is shown.'),
        findsOneWidget,
      );
    });
  });
  ...
  group('DirectoryEntry helpers', () {
    test('visibleEntries hides hidden by default', () {
      ...
      expect(visibleEntries(entries, null, false, null).map((e) => e.name), [
        'b',
      ]);
      expect(visibleEntries(entries, null, true, null).map((e) => e.name), [
        '.a',
        'b',
      ]);
    });
    ...
    test('dot prefix reveals hidden matches', () {
      ...
      expect(visibleEntries(entries, null, false, '.co').map((e) => e.name), [
        '.config',
      ]);
    });
  ...
  group('AdaptiveDirectoryPicker platform contracts', () {
    testWidgets('WebDirectoryPickerField shows Miller browser hint', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(WebDirectoryPickerField(value: null, onPicked: (_) {})),
      );
      expect(find.textContaining('Miller-column browser'), findsOneWidget);
      expect(find.text('Browse'), findsOneWidget);
    });

    testWidgets('MacDirectoryPickerField shows native picker hint', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(MacDirectoryPickerField(value: '/work/main', onPicked: (_) {})),
      );
      expect(find.textContaining('native folder picker'), findsOneWidget);
      expect(find.text('/work/main'), findsOneWidget);
    });

    testWidgets('Web field Browse opens Miller dialog (host reachable)', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        wrap(WebDirectoryPickerField(dialogTitle: 'Pick', onPicked: (_) {})),
      );
