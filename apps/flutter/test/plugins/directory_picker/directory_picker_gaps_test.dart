...
void main() {
  group('Gaps fixed: directory picker completion', () {
    test('slot holes are single root kind per contract', () async {
      ...
      const specSidebar = SlotSpec(kind: SlotKind.single, scope: SlotScope.root);
      expect(specHero.kind, SlotKind.single);
      expect(specHero.scope, SlotScope.root);
      expect(specSidebar.kind, SlotKind.single);
    });

    testWidgets('browse and native flows are SlotWidgetBuilders via directory_picker_flows', (tester) async {
      ...
      final browse = BrowseDirectoryFlow(
        ...
        injected: BrowseFlowInjected(
          listDirectory: ({String? path, DirectoryListSignal? signal}) async => _listingFor(path),
    ...
    testWidgets('aria-current: selected row carries Semantics selected', (tester) async {
      await tester.pumpWidget(_wrap(DirectoryBrowser(
        open: true,
        listDirectory: ({String? path, DirectoryListSignal? signal}) async => _listingFor(path),
        createDirectory: ({required String path, required String name}) async => '$path/$name',
        onOpen: (_) {},
      ...
      final selectedSemantics = find.byWidgetPredicate((w) => w is Semantics && w.properties.selected == true);
    ...
    test('IME isComposing guard exists in DirectoryBrowser', () async {
      final source = await File('lib/src/plugins/directory_picker/directory_browser.dart').readAsString();
    ...
    test('DirectoryListSignal aborts host scan (abort semantics)', () async {
      bool called = false;
      Future<Map<String, Object?>> fakeList({String? path, DirectoryListSignal? signal}) async {
        called = true;
        final sig = signal ?? DirectoryListSignal();
        // Simulate scan that checks abort
        await Future<void>.delayed(const Duration(milliseconds: 20));
        if (sig.aborted) throw Exception('aborted');
        return {'path': path ?? _home, 'home': _home, 'crumbs': [], 'entries': [], 'truncated': false};
      }
    ...
    test('locale dictionaries contain expected keys (fallback parity)', () {
      expect(kDirectoryBrowserEn['browser.title'], 'Select Workspace Directory');
    ...
    testWidgets('slot holes are declared and occupied after host activation', (tester) async {
            ...
            home: Consumer(
              builder: (context, ref, _) {
                ...
                Future.microtask(() async {
                  ...
                  expect(host.slots.isDeclared('conversation.hero.workspace.directoryFlow'), isTrue);
                  expect(host.slots.isDeclared('sidebar.workspaces.directoryFlow'), isTrue);
                  // At least one occupant per hole (browse on web, native on VM)
                  expect(host.slots.entries('conversation.hero.workspace.directoryFlow').length, greaterThanOrEqualTo(1));
                  expect(host.slots.entries('sidebar.workspaces.directoryFlow').length, greaterThanOrEqualTo(1));
                  // Locale namespace registered by BrowseDirectoryPickerPlugin
                  final locale = host.service<LocaleService>('locale')!;
                  expect(locale.bind('directory-browser')('browser.title'), isNotEmpty);
                });
