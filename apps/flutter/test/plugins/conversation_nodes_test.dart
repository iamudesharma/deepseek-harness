...
void main() {
  group('ConversationNodeFolder determinism', () {
    final script = [
      ...
      _event('assistant/chunk', 3, {
        'turn': 1,
        'step': 1,
        'chunk': {'text': 'Hel'},
      }),
      _event('assistant/chunk', 4, {
        'turn': 1,
        'step': 1,
        'chunk': {'text': 'lo'},
      }),
    ...
    test(
      'streaming tail updates in-flight node until settled message lands',
      () {
        final folder = ConversationNodeFolder()..forEach(script.take(4));
        expect(
          folder.toLines().where((l) => l.startsWith('assistant')).single,
    ...
    test(
      'tool lifecycle: running → result settles with status + payload + seqs',
      () {
    ...
    test('code-dispatch folds nested subcalls under the root call', () {
      final folder = ConversationNodeFolder()
        ..add(_event('turn/start', 1, {'turn': 1}))
        ..add(_event('tool/call', 2, {'callId': 'root-1', 'name': 'run-code'}))
        ..add(
          _event('tool/code-dispatch-start', 3, {
            'rootCallId': 'root-1',
            'parentCallId': 'root-1',
        ...
        ..add(
          _event('tool/code-dispatch-start', 5, {
            'rootCallId': 'root-1',
            'parentCallId': 'root-1',
    ...
    test('nested subcalls depth 2 and 3 are folded recursively', () {
      final folder = ConversationNodeFolder()
        ...
        ..add(
          _event('tool/code-dispatch-start', 4, {
            'rootCallId': 'root-1',
        ...
        ..add(
          _event('tool/code-dispatch-start', 6, {
            'rootCallId': 'root-1',
    ...
    test('turn error folds friendly copy through displayFailureMessage', () {
      ...
      final errorLine = folder.toLines().singleWhere(
        (l) => l.startsWith('turn-error'),
      );
  ...
  group('refusal and extensions', () {
    test('unknown required event refuses reconstruction', () {
      ...
      expect(
        ...
        throwsA(
          predicate((e) => e is StateError && e.message.contains('refusing')),
        ),
      );
    });
  ...
  group('step grouping and summary', () {
    test('events fold into a StepGroupNode with computed summary', () {
      ...
      expect(
        g.children,
        hasLength(2),
      ); // assistant + tool call (result settles it)
  ...
  group('compaction surface-replace', () {
    test('summary replaces bracket-compact range and cites shadowed seqs', () {
      ...
      expect(compactions.single.sourceSeqs.contains(4), isFalse);
    });
    ...
    test(
      'compact checkpoint without pending still creates marker (window cut)',
      () {
        final folder = ConversationNodeFolder()
          ..add(_event('user/message', 1, {'content': 'keep'}, sources: []))
          ..add(
    ...
    test('legacy bracket-trio behavior is preserved without shadowedSeqs', () {
      ...
      expect(nodes.whereType<UserMessageNode>().map((n) => n.text), ['go']);
      expect(
        nodes.whereType<CompactionNode>().single.text,
        'condensed history',
      );
    });
  });
