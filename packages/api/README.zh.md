...
| [`terminal-controller/`](terminal-controller/README.zh.md) | 拥有基于持久 PTY 服务的 console 终端 Remote 命令。 | `ctx.terminalController` / `ctx.remote.terminal` |

Remote 调用沿 Client → Host 方向运行在应用共享的 Connection 之上。API Gateway 拥有 Remote 传输，各控制器包分别拥有 Session、配置界面、Workspace 与 console 终端行为。流式下载等不适合 Remote 调用的响应由功能包注册精确的 Connection Fetch 路由。
