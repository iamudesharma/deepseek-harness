...
shell 在整个生命周期内运行在有效的沙箱边界之下。agent 所有者从其会话解析边界——会话 cwd、会话模式覆盖，再到部署默认值——并且当该所有者仍有打开的会话或进行中的 spawn 时，改变有效沙箱模式会被拒绝——请先等待创建完成并关闭会话，避免以更宽权限打开的终端在权限降级后继续存在。console 所有者（客户端终端主体）没有会话：它以无 agent 方式解析，落到部署默认模式与配置的 workspace 根，并且不携带模式限制，因为不存在属于它的会话可供改变。后端只提供终端专属的环境覆盖；共享凭据清理由子进程提供方负责。
...
当 agent 所有者存在打开的会话或进行中的 spawn 时，凡是会改变有效沙箱模式的写入都会在 `sandbox/mode` 事件提交前被拒绝。该限制绑定到确切所有者，并在保留现有会话的提供方重新加载后依然有效；console 所有者没有可改变模式的会话，不安装限制。
