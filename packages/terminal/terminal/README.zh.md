...
`dsh-terminal` 为 harness 提供持久且限定所有者范围的终端会话：会话让 shell 或 REPL 状态跨工具调用存活，且每个操作都被限制在创建它的那个确切所有者内。本包提供 `ctx.terminals` 服务，负责生成不透明的会话 id、通过已注册的后端路由会话创建，并在所有者或服务 dispose（资源释放）时等待完全停稳的清理。它本身不定义任何终端机制：`dsh-terminal-bash` 之类的后端负责启动与就绪检测，`dsh-tool-terminal` 中的面向模型工具负责呈现。会话只存在于进程本地：harness 重启后不会恢复。
...
每个会话都由打开它的确切所有者拥有，以可辨识的 `TerminalOwner` 联合表达：agent 所有者包装确切存活的 `Agent`（面向模型的面，其存活性取决于 Agent 注册表条目），console 所有者则是没有模型会话的客户端终端主体，自带一个 effect 作用域（其存活性取决于该作用域的 dispose）。凡是指名会话的操作，只要调用方不是拥有该会话的权威对象就会被拒绝，因此即使获知 id，一个所有者也无法操作另一个所有者的终端。可选的会话 `name` 是所有者本地的显示元数据——例如 `main` 或 `gdb` 这样的标签——并且只在所有者范围内唯一。

### 可观察结果与失败

成功打开会返回会话 id、类型、后端提供的 pid（如有）、状态与有界启动消息。发送以等待原因结算：`stdin_read`（shell 正在等待输入）、`inferred_idle`（输出静默）、`timeout` 或 `session_exit`（顶层 shell 已退出）。失败携带稳定的机器可路由错误码：后端类型缺失（`NO_BACKEND`）、会话未知（`NO_SESSION`）、属于其他所有者的会话（`FOREIGN_SESSION`）、并发第二次发送（`SEND_ACTIVE`），或所有者不再存活（`OWNER_NOT_LIVE`）。后端设置失败会在发布任何内容之前拒绝打开；清理失败会拒绝关闭，而不是声称成功。
...
- 限制比较拥有会话的权威对象——agent 所有者为 `Agent`，console 所有者为 principal 对象——因此同一权威的重新包装所有者仍然匹配。`hasOwnerActivity(owner)` 覆盖从尚未发布的设置到最终关闭的全过程，没有发布竞态，因此生命周期策略可以精确限制所有者。
