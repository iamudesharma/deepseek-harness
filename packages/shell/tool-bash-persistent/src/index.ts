...
import type { AgentTerminalOwner, TerminalReadResult, TerminalSessionId } from '@deepseek-ai/dsh-terminal'
...
function persistentShells(ctx: Context, config: ResolvedConfig): PersistentShells {
  ...
  ctx.effect(() => async () => {
    ...
    const closing = [...live.values()].map(async (entry) => { await close(entry.owner, entry.id, 'tool-bash-persistent disposed') })
...
function registerPersistentBash(ctx: Context, config: ResolvedConfig): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (exec.agent === undefined) throw new Error('bash requires an owning agent session')
